...
from wan.utils.utils import best_output_size, merge_video_audio, save_video, str2bool
...
def _parse_args():
    ...
    parser.add_argument(
        "--physical_feature_path",
        type=str,
        default=None,
        help="Path to a .pt physical feature file to inject.")
    parser.add_argument(
        "--physical_injection_layers",
        type=str,
        default="0,1,2",
        help="Comma-separated injection layer indices (e.g., 0,1,2).")
    parser.add_argument(
        "--physical_adapter_dim",
        type=int,
        default=16,
        help="Adapter output dim (must match latent channels).")
    parser.add_argument(
        "--physical_num_queries",
        type=int,
        default=128,
        help="Number of learnable queries in PhysicalAdapter.")
    parser.add_argument(
        "--physical_ckpt",
        type=str,
        default=None,
        help="Path to a trained physical checkpoint (merged_model.pt).")
    parser.add_argument(
        "--prompt",
        type=str,
        default=None,
...
def _parse_layer_list(raw_layers):
    return [int(x) for x in raw_layers.split(',') if x.strip()]


def _load_physical_features(path):
...
def _load_physical_checkpoint(path):
...
def _compute_target_shape_and_seq_len(frame_num, size, vae_stride, patch_size, sp_size, z_dim):
    ...
    seq_len = math.ceil((h_lat * w_lat) / (patch_size[1] * patch_size[2]) * f_lat / sp_size) * sp_size
    target_shape = torch.Size([1, z_dim, f_lat, h_lat, w_lat])
    return target_shape, int(seq_len)


def _maybe_enable_physical_injection(pipe, args, size, img=None, max_area=None):
    if args.physical_feature_path is None:
        return None
    ...
    ref_features = _load_physical_features(args.physical_feature_path)

    if img is not None and max_area is not None:
    ...
    injection_layers = _parse_layer_list(args.physical_injection_layers)
    ...
    if args.physical_ckpt:
        ...
        logging.info(
            f"Loaded physical checkpoint: missing={len(missing)} unexpected={len(unexpected)}")

    pipe.physical_injection_manager = manager
    logging.info(
        f"Physical injection enabled: layers={injection_layers}, seq_len={seq_len}, target_shape={target_shape}")
    return manager
...
def generate(args):
    ...
    if "t2v" in args.task:
        ...
        _maybe_enable_physical_injection(
            pipe=wan_t2v,
            args=args,
